...
impl AppImpl {
    ...
    pub fn delete_feed(&mut self) -> Result<()> {
        if matches!(self.selected, Selected::Feeds) && matches!(self.mode(), Mode::Editing) {
            ...
            crate::rss::delete_feed(&self.conn, feed_id)?;
            ...
            while j < self.entries.items.len() {
                if self.entries.items[j].feed_id == feed_id {
                    self.entries.items.remove(j);
                } else {
                    j += 1;
                }
            }
    ...
    fn open_link_in_browser(&self) -> Result<()> {
        webbrowser::open(&self.get_current_link()).map(|_| ()).map_err(|e| anyhow::anyhow!(e))
    }
